...
func connect_to_channel(CHANNEL, CLIENT_ID, CLIENT_PASSWORD, CLIENT_NICK, CLIENT_REALNAME):
	...
	queue_message('USER ' + CLIENT_ID + ' ' + HOST + ' ' + HOST + ' ' + CLIENT_REALNAME)
	queue_message('JOIN #' + CHANNEL)
	
	printt("GodotIRC: Connecting to channel:", CHANNEL)
	
	CURRENT_CHANNEL = CHANNEL


func disconnect(a, b, c):
	set_process(false)
...
func _process(delta):
	...
	if Stream.get_status() == StreamPeerTCP.STATUS_ERROR:
		print("GodotIRC: Disconnected from host. Retrying.")
...
func send_message(text):
	send_command(str("PRIVMSG #", GodotIRC.IRC_CHANNEL, " :", text))
...
func _on_text_recieved(data):
	GodotIRC.emit_signal("message_recieved", data["message"])
	GodotIRC.emit_signal("message_recieved_raw", data)
